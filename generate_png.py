...
#
    ...
    return aggregate_attention(attention_store,16,['down','up'],is_cross=True,select=0)
...
if __name__=='__main__':
    ...
    for idx,i in tqdm(enumerate(range(len(data))),total=len(data)):
        ...
        for idx,p in enumerate(sentences):
            ...
            cross_16 = save_multi_scale_attention_map(controller,tag_id,idx)
            filename= f'{tag_id}/cross16_{idx}.png' 
            pdb.set_trace()
            save_visualize_cross_attn_map(cross_16, tokenizer.decode, clip_tokens_ids, filename)
