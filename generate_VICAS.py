...
def save_multi_scale_attention_map(attention_store: AttentionStore, save_dir, frame_id, idx=None):
    import os.path as osp
    os.makedirs(save_dir, exist_ok=True)

    for scale in [16, 32, 64]:
        subdir = osp.join(save_dir, f"cross_{scale}")
        os.makedirs(subdir, exist_ok=True)
        save_path = osp.join(subdir, f"{frame_id}.pt")

        if not osp.exists(save_path):
            cross_attn = aggregate_attention(attention_store, scale, ['down', 'up'], is_cross=True, select=0)
            save_torch_tensor(cross_attn, save_path)

    for scale in [16, 32, 64]:
        subdir = osp.join(save_dir, f"self_{scale}")
        os.makedirs(subdir, exist_ok=True)
        save_path = osp.join(subdir, f"{frame_id}.pt")

        if not osp.exists(save_path):
            self_attn = aggregate_attention(attention_store, scale, ['down', 'up'], is_cross=False, select=0)
            save_torch_tensor(self_attn, save_path)
...
def save_visualize_cross_attn_map(cross_attn,decoder,tokens,file_name):
    ...
    for j in range(len(tokens)):
        ...
        # vis_img = ptp_utils.text_under_image(vis_img, tokens[j])
        vis_img = np.array(vis_img)
        vis_images.append(vis_img)
    output_path = f'./outputs/nulltext_png/{file_name}.png'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    view_images(np.concatenate(vis_images, axis=1)).save(output_path)
...
if __name__=='__main__':
    ...
    for idx, vid in tqdm(enumerate(val_ids), total=len(val_ids), desc="Processing videos"):
        ...
        if not osp.exists(anno_path):
            print(f"Warning: {anno_path} not found, skip.")
            continue
        ...
        video_id = anno_data.get("video_id", None)
        filename = anno_data.get("filename", "")
        caption = anno_data.get("caption_parsed_en_gpt", "")

        frame_dir = osp.join(frames_root, vid_str)
        if not osp.exists(frame_dir):
            print(f"Warning: frames not found for video {vid_str}")
            continue

        frame_list = sorted([
            osp.join(frame_dir, f)
            for f in os.listdir(frame_dir)
            if f.lower().endswith((".jpg", ".png"))
        ])

        if len(frame_list) == 0:
            print(f"No frames in {frame_dir}, skip.")
            continue

        tokens = split_text(caption)
        clip_tokens_ids = ldm_stable.tokenizer(caption)['input_ids']
        ...
        if len(clip_tokens) <= 75:
            ...
            sentences = []
            start = 0
            clip_tokens_ids_valid = clip_tokens_ids[1:-1]
            for i in range(len(splited_tokens)):
                print(start, 'end={}'.format(start + len(splited_tokens[i])))
                sentences += [ldm_stable.tokenizer.decode(
                    [clip_tokens_ids[0]] +
                    clip_tokens_ids_valid[start:start + len(splited_tokens[i])] +
                    [clip_tokens_ids[-1]]
                )[15:-14]]
                start += len(splited_tokens[i])

        save_dir = osp.join(save_root, vid_str)
        os.makedirs(save_dir, exist_ok=True)
        
        # frame_indices = list(range(0, len(frame_list), 10))
        frame_indices = list(range(len(frame_list)))
        for f_idx in tqdm(frame_indices, desc=f"video {vid_str}", leave=False):
            frame_path = frame_list[f_idx]
            frame_name = osp.basename(frame_path)   # e.g. "00005.jpg"
            frame_id = osp.splitext(frame_name)[0]  # e.g. "00005"

            save_dir_1 = osp.join(save_dir, frame_id)
            if osp.exists(save_dir_1):
                continue
            
            for s_idx, prompt in enumerate(sentences):
                tag_id = f"{vid_str}_{frame_id}"
            ...
            save_multi_scale_attention_map(controller, save_dir_1, frame_id)