...
def load_video_frames(data_path, split):
    ...
    with open(data_path) as f:
        ...
        for video, video_data in videos.items():
            ...
            expressions = video_data["expressions"]  # dict: exp_id -> {exp, obj_id}
...
def deduplicate_frames(frames):
    ...
    unique = []
    for frame in frames:
        key = (frame["video"], frame["frame"], frame["obj_id"])
        if key not in seen:
            seen.add(key)
            unique.append(frame)
    return unique
...
if __name__ == '__main__':
    ...
    # 读取 ref-davis 的 valid 数据
    valid_frames = load_video_frames(
        "datasets/ref-davis/meta_expressions/valid/meta_expressions.json", split="valid"
    )

    # 分布式划分样本
    filtered_frames = [
        frame for idx, frame in enumerate(valid_frames) if idx % world_size == local_rank
    ]

    null_inversion = NullInversion(ldm_stable)

    filtered_frames = deduplicate_frames(filtered_frames)

    for frame in tqdm(filtered_frames, total=len(filtered_frames)):
        video = frame["video"]
        ...
        output_path = f"./outputs/attn/{video}/{obj_id}/"
        os.makedirs(output_path, exist_ok=True)

        # 图像路径
        image_path = osp.join(
            f"datasets/ref-davis/{split}/JPEGImages/{video}", f"{frame_id}.jpg"
        )
        assert osp.exists(image_path), f"Image not found: {image_path}"

        # Token处理
        clip_token_ids = tokenizer(expression)["input_ids"]
        clip_tokens = [tokenizer.decode(i) for i in clip_token_ids[1:-1]]  # 去除 [BOS]/[EOS]
        ...
        if len(clip_tokens) <= 75:
            ...
            sentences = []
            start = 0
            token_ids_valid = clip_token_ids[1:-1]
            for s in splited_tokens:
                token_slice = token_ids_valid[start:start + len(s)]
                ids = [clip_token_ids[0]] + token_slice + [clip_token_ids[-1]]
                sentence = tokenizer.decode(ids)[15:-14]
        ...
        image_inv, x_t = run_and_display(
            ...
            verbose=False,
            file_name=f"{frame_id}"  # 用frame_id命名
        )
        save_multi_scale_attention_map(controller, output_path, frame_id)